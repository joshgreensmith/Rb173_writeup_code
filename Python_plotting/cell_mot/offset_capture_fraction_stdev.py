...
with open('cell_mot_output/offsets_run_3.csv', 'r') as f:
  ...
  for line in reader:
    ...
    capture_fractions_stdevs.append(np.std(literal_eval(line['capture fraction array'])))

plt.plot(offset_deltas, capture_fractions_stdevs)

plt.xlabel('offset deltas (m)')
plt.ylabel('capture fraction stdev')

plt.title('Capture fraction stdev against offset delta for detuning -12MHz')
