...
fn main() {
    ...
    builder = builder.with(
      file::new::<Position, Text>("cell_mot_output/pos.txt".to_string(), 1),
      "",
      &[],
    );
    ...
    let half_width = Vector3::new(0.17, 0.1, 0.1);
    world
        .create_entity()
    ...
    // Add atoms
    for z in [0.0, 0.5e-3, 1.0e-3, 1.5e-3, 2.0e-3, 2.5e-3, 3.0e-3, 3.5e-3] {
      world
          .create_entity()
          .with(Position {
              pos: Vector3::new(
